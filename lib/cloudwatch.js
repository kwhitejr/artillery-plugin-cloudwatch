...
const getQuantileMetrics = require('./quantile');
...
const impl = {
  validateConfig(scriptConfig) {
    ...
    } else if (!(typeof scriptConfig.plugins[constants.PLUGIN_NAME][constants.PLUGIN_PARAM_NAMESPACE] === 'string'
  ...
  buildLatencyMetrics(report) {
    const latencies = report._latencies;
    const timestamp = Math.max.apply(null, report._requestTimestamps);
    const dimensions = [
      {
        Name: 'CATEGORY',
        Value: 'performance'
  ...
  buildStatusCodeMetrics(report) {
    const statusCodes = report._codes;
    const timestamp = Math.max.apply(null, report._requestTimestamps);
    const dimensions = [
      {
        Name: 'CATEGORY',
    ...
    const metrics = Object.entries(statusCodes)
      .map(([statusCode, amount]) => ({
  ...
  CloudWatchPlugin(scriptConfig, eventEmitter) {
    const self = this;
    ...
    eventEmitter.on('stats', (report) => {
      ...
      const statusCodeMetrics = impl.buildStatusCodeMetrics(report);
      // build new metrics here

      const cloudWatchParams = {
        Namespace: self.config[constants.PLUGIN_PARAM_NAMESPACE],
        MetricData: [].concat(latencyMetrics, statusCodeMetrics) // concat new metrics here
      };
